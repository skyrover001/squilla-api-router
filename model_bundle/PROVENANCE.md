...
`squilla_router_server.runtime.RouterRuntime`.

## Purpose

The bundle provides the V4 Phase 3 local model router used to classify a turn
into route classes `R0` through `R3`. Concrete model mapping belongs to an
external forwarding adapter and is not included in this standalone profile.
This provenance file does not change runtime behavior; it records the assets
that the runtime loads.
...
The upstream MIT license and attribution must be retained in standalone release
notices when this bundle is distributed.
...
1. Update `artifact_manifest.json` with the new size and SHA-256 values.
2. Review the changed `artifact_manifest.json` entries.
3. Run `uv run pytest tests/test_runtime_api.py -q`.
