...
from squilla_api_router.v4_runtime.features import ContextMetadata
from squilla_api_router.v4_runtime.flags import compute_flags
from squilla_api_router.v4_runtime.inference.types import (
    FinalDecision,
    InferenceRequest,
)
from squilla_api_router.v4_runtime.policy import (
    ROUTE_CLASSES,
    RouteDecision,
...
def _normalize_history(prev_route_decisions: list[Any]) -> list[RouteDecision]:
    normalized: list[RouteDecision] = []
    for item in prev_route_decisions or []:
        if hasattr(item, "route_class"):
            normalized.append(cast(RouteDecision, item))
        elif isinstance(item, dict) and "route_class" in item:
            normalized.append(
                cast(RouteDecision, SimpleNamespace(route_class=item["route_class"]))
            )
        else:
            raise ValueError(
