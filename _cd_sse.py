# Vendored + adapted from codex-deepseek (Apache-2.0):
# https://github.com/codex-deepseek/codex-deepseek
# Chat streaming chunks -> OpenAI Responses SSE events.
import json
import os
import random
import string
...
from squilla_api_router._cd_log import log
...
class SseTranslator:
    ...
    def feed(self, chunk: dict) -> str:
        ...
        output = ""
        # Some backends (tianhe Qwen thinking mode) stream only `reasoning`
        # deltas with content=null; surface that as the visible answer too.
        content_text = delta.get("content") or delta.get("reasoning")
        if content_text:
            output += self._ensure_started()
            self.content_so_far += content_text
